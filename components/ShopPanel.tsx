      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredItems.map(item => {
          ...
          return (
            <div 
              key={item.id} 
              className={`group relative bg-white dark:bg-slate-900 rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-2 hover:shadow-xl border ${
            ...
            >
              ...
              <div className="p-5">
                <div className="flex justify-between items-start mb-2">
                    <h3 className={`font-bold text-lg leading-tight ${premium ? 'text-yellow-700' : 'text-gray-800 dark:text-slate-100'}`}>{item.name}</h3>
                    {premium && <Crown size={16} className="text-yellow-600 shrink-0 mt-1" />}
                </div>
                
                <p className="text-xs text-gray-500 dark:text-slate-300 mb-6 line-clamp-2 h-8">{item.description}</p>
        ...
        <div className={`fixed inset-0 z-50 flex items-center justify-center ${modalAnimateIn ? 'bg-black/50' : 'bg-black/0'}` }>
          <div className={`w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl transform transition-all duration-300 ${modalAnimateIn ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
            ...
            <div className="p-4 bg-white dark:bg-slate-900 text-right border-t border-gray-100 dark:border-slate-800">
              <button onClick={() => { setModalAnimateIn(false); setTimeout(() => setShowPremiumModal(false), 280); }} className="text-sm text-indigo-900 dark:text-slate-100 font-semibold">Cerrar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {(showPointsModal || pointsModalAnimateIn) && (
        <div className={`fixed inset-0 z-50 flex items-center justify-center ${pointsModalAnimateIn ? 'bg-black/50' : 'bg-black/0'}` }>
          <div className={`w-full max-w-4xl rounded-3xl overflow-hidden shadow-2xl transform transition-all duration-300 ${pointsModalAnimateIn ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
              ...
              <div className="grid md:grid-cols-3 gap-6">
                <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-xl border border-gray-100 dark:border-slate-800 text-left">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <h4 className="font-bold text-slate-800 dark:text-slate-100">Puntos</h4>
                  </div>
                  <p className="text-sm text-slate-600 dark:text-slate-300">Los puntos se obtienen al enviar reportes y se usan para canjear fondos, marcos e insignias en la tienda.</p>
                  <ul className="mt-3 text-slate-600 dark:text-slate-300 list-disc list-inside space-y-1 text-sm">
                ...
                <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-xl border border-gray-100 dark:border-slate-800 text-left">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <h4 className="font-bold text-slate-800 dark:text-slate-100">XP y Niveles</h4>
                  </div>
                  <p className="text-sm text-slate-600 dark:text-slate-300">La experiencia "XP" refleja tu actividad. Subiendo niveles desbloqueas etiquetas cada 20 niveles.</p>
                  <ul className="mt-3 text-slate-600 dark:text-slate-300 list-disc list-inside space-y-1 text-sm">
                ...
                <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-xl border border-gray-100 dark:border-slate-800 text-left">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <h4 className="font-bold text-slate-800 dark:text-slate-100">Tienda</h4>
                  </div>
                  <p className="text-sm text-slate-600 dark:text-slate-300">Canjea tus puntos por personalizaciones. Algunos artículos requieren Premium.</p>
                  <ul className="mt-3 text-slate-600 dark:text-slate-300 list-disc list-inside space-y-1 text-sm">
            ...
            <div className="p-4 bg-white dark:bg-slate-900 text-right border-t border-gray-100 dark:border-slate-800">
              <button onClick={() => { setPointsModalAnimateIn(false); setTimeout(() => setShowPointsModal(false), 280); }} className="text-sm text-indigo-900 dark:text-slate-100 font-semibold">Cerrar</button>
            </div>
          </div>
        </div>
      )}
