  ...
  return (
    <div className="max-w-6xl mx-auto animate-fade-in space-y-8 pb-12">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-primary to-blue-900 p-8 text-white shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-3xl font-extrabold tracking-tight">{t('shop.rewards_shop', 'Tienda de Recompensas')}</h2>
              <p className="text-blue-200 font-light">{t('shop.redeem_points', 'Canjea tus puntos por personalizaciones exclusivas')}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 bg-black/30 p-4 pr-6 rounded-2xl backdrop-blur-sm border border-white/10 shadow-inner">
            ...
            <div className="text-right">
              <p className="text-xs text-yellow-100 uppercase font-bold tracking-wider">{t('shop.your_balance', 'Tu Saldo')}</p>
        ...
        <div className="bg-gradient-to-r from-indigo-900 to-primary rounded-3xl p-4 shadow-2xl text-white flex flex-col sm:flex-row items-center gap-4 border border-white/10">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-sm text-white/80">{t('premium.exclusive_benefits', 'Fondos exclusivos, etiquetas únicas y soporte prioritario.')}</div>
            </div>
          </div>
          ...
          <div className="hidden sm:flex items-center gap-4 ml-auto">
            <div className="flex items-center gap-3 bg-white/5 rounded-full px-3 py-2">
              ...
              <span className="text-sm text-white/90">{t('premium.exclusive_content', 'Contenido exclusivo')}</span>
            </div>
            <div className="flex items-center gap-3 bg-white/5 rounded-full px-3 py-2">
              <Trophy className="text-yellow-300" />
              <span className="text-sm text-white/90">{t('premium.premium_tags', 'Etiquetas premium')}</span>
            </div>
          </div>
          ...
          <div className="w-full sm:w-auto flex justify-center sm:justify-end">
            {(() => {
              ...
              return (
                ...
                >
                  {isPremiumUser ? t('premium.view_benefits', 'Ver mis ventajas') : t('premium.get_premium', 'Quiero Premium')}
                </button>
              );
            })()}
          </div>
      ...
      <div className="flex justify-center">
        <div className="bg-white dark:bg-slate-800 p-1.5 rounded-full shadow-lg border border-gray-100 dark:border-gray-700 inline-flex relative transition-colors">
          <button
            ...
            className={`px-6 py-2.5 rounded-full text-sm font-bold flex items-center gap-2 transition-all duration-300 ${activeTab === 'frame'
                ? 'bg-primary dark:bg-blue-600 text-white shadow-md transform scale-105'
                : 'text-gray-500 dark:text-gray-400 hover:text-primary dark:hover:text-white hover:bg-gray-50 dark:hover:bg-slate-700'
              }`}
          >
            <Square size={18} /> {t('shop.frames', 'Marcos')}
          </button>
          <button
          ...
          >
            <Layout size={18} /> {t('shop.backgrounds', 'Fondos')}
          </button>
          <button
          ...
          >
            <Trophy size={18} /> {t('shop.badges', 'Etiquetas')}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredItems.map(item => {
          ...
          return (
            <div
              key={item.id}
              className={`group relative bg-white dark:bg-slate-800 rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-2 hover:shadow-xl border ${premium ? 'border-yellow-100 dark:border-yellow-900/30 shadow-yellow-100/50' : 'border-gray-100 dark:border-gray-700 shadow-sm'
                }`}
            >
              {premium && (
              ...
              <div className={`h-40 relative flex items-center justify-center overflow-hidden ${premium ? 'bg-slate-50' : 'bg-gray-50'}`}>
                ...
                {!isOwned && (
                  <div className="absolute bottom-2 right-2 bg-white/90 dark:bg-slate-800/90 backdrop-blur text-primary dark:text-white px-2.5 py-1 rounded-lg text-xs font-bold shadow-sm border border-gray-100 dark:border-gray-700 flex items-center gap-1">
                    {item.cost === 0 ? t('shop.free', 'Gratis') : item.cost}
              ...
              <div className="p-5">
                <div className="flex justify-between items-start mb-2">
                  <h3 className={`font-bold text-lg leading-tight ${premium ? 'text-yellow-700 dark:text-yellow-400' : 'text-gray-800 dark:text-white'}`}>{item.name}</h3>
                  {premium && <Crown size={16} className="text-yellow-600 shrink-0 mt-1" />}
                </div>

                <p className="text-xs text-gray-500 dark:text-gray-400 mb-6 line-clamp-2 h-8">{item.description}</p>

                <div className="mt-auto">
                  {isOwned ? (
                    isEquipped ? (
                      <button disabled className="w-full py-2.5 bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 border border-green-200 dark:border-green-800 rounded-xl font-bold text-sm cursor-default flex items-center justify-center gap-2">
                        <Check size={16} /> {t('shop.equipped', 'Equipado')}
                      </button>
                    ) : (
                      <button
                        onClick={() => onEquip(item)}
                        className="w-full py-2.5 bg-white dark:bg-slate-700 text-gray-700 dark:text-white border-2 border-gray-200 dark:border-gray-600 rounded-xl font-bold text-sm hover:border-primary dark:hover:border-blue-500 hover:text-primary dark:hover:text-blue-400 transition-colors"
                      >
                    ...
                    premium && !user.premium ? (
                      ...
                      >
                        <Lock size={14} /> {t('shop.premium_only', 'Solo Premium')}
                      </button>
                    ) : (
                      <button
                      ...
                      >
                        ...
                        {canAfford ? t('shop.buy', 'Comprar') : t('shop.insufficient_points', 'Faltan Puntos')}
                      </button>
                    )
                  )}
                </div>
        ...
        <div className={`fixed inset-0 z-50 flex items-center justify-center ${modalAnimateIn ? 'bg-black/50' : 'bg-black/0'}`}>
          <div className={`w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl transform transition-all duration-300 ${modalAnimateIn ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
                ...
                <div>
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-yellow-200">{t('premium.upgrade_desc', 'Mejora tu experiencia y desbloquea contenido exclusivo')}</p>
                    </div>
                  </div>
                    ...
                    <div className="bg-gradient-to-r from-emerald-50 to-emerald-100 border border-emerald-200 rounded-2xl p-5 text-center mx-auto shadow-lg">
                      <div className="flex items-center justify-center mb-3">
                        ...
                        <div className="text-left">
                          <div className="text-sm font-semibold text-emerald-800">{t('premium.thanks', '¡Gracias por adquirir Premium!')}</div>
                          <div className="text-xs text-emerald-700">{t('premium.account_updated', 'Tu cuenta ha sido actualizada.')}</div>
                        </div>
                      </div>
                      <div className="text-sm text-gray-700">{t('premium.enjoy', 'Disfruta de fondos y etiquetas exclusivas, bonificaciones y soporte prioritario.')}</div>
                    </div>
                  ) : (
                    <div className="bg-white/10 rounded-xl p-4 text-center mx-auto">
                      <div className="text-sm text-white/80">{t('premium.price', 'Precio')}</div>
                      ...
                      >{`${t('premium.buy_for', 'Comprar por')} ${PREMIUM_COST_POINTS} pts`}</button>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="p-4 bg-white text-right">
              <button onClick={() => { setModalAnimateIn(false); setTimeout(() => setShowPremiumModal(false), 280); }} className="text-sm text-indigo-900 font-semibold">{t('common.close', 'Cerrar')}</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className={`w-full max-w-4xl rounded-3xl overflow-hidden shadow-2xl transform transition-all duration-300 ${pointsModalAnimateIn ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
            <div className="p-6 bg-gradient-to-r from-indigo-900 to-primary text-white">
              <div className="mb-6 text-center">
                <h3 className="text-2xl font-extrabold">{t('shop.how_it_works', '¿Cómo funcionan los Puntos y la XP?')}</h3>
                <p className="text-white/80 max-w-2xl mx-auto mt-2">{t('shop.how_it_works_desc', 'Resumen rápido de cómo ganar puntos, subir de nivel y canjearlos en la tienda.')}</p>
              </div>
              ...
              <div className="grid md:grid-cols-3 gap-6">
                <div className="bg-white p-6 rounded-3xl shadow-xl border border-gray-100 text-left">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <h4 className="font-bold text-slate-800">{t('shop.points', 'Puntos')}</h4>
                  </div>
                  <p className="text-sm text-slate-600">{t('shop.points_desc', 'Los puntos se obtienen al enviar reportes y se usan para canjear fondos, marcos e insignias en la tienda.')}</p>
                ...
                <div className="bg-white p-6 rounded-3xl shadow-xl border border-gray-100 text-left">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <h4 className="font-bold text-slate-800">{t('shop.xp_levels', 'XP y Niveles')}</h4>
                  </div>
                  <p className="text-sm text-slate-600">{t('shop.xp_desc', 'La experiencia "XP" refleja tu actividad. Subiendo niveles desbloqueas etiquetas cada 20 niveles.')}</p>
                  <ul className="mt-3 text-slate-600 list-disc list-inside space-y-1 text-sm">
                ...
                <div className="bg-white p-6 rounded-3xl shadow-xl border border-gray-100 text-left">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <h4 className="font-bold text-slate-800">{t('shop.store', 'Tienda')}</h4>
                  </div>
                  <p className="text-sm text-slate-600">{t('shop.store_desc', 'Canjea tus puntos por personalizaciones. Algunos artículos requieren Premium.')}</p>
                  <ul className="mt-3 text-slate-600 list-disc list-inside space-y-1 text-sm">
            ...
            <div className="p-4 bg-white text-right">
              <button onClick={() => { setPointsModalAnimateIn(false); setTimeout(() => setShowPointsModal(false), 280); }} className="text-sm text-indigo-900 font-semibold">{t('common.close', 'Cerrar')}</button>
            </div>
          </div>
        </div>
      )}
