...
const IssueDetailModal: React.FC<IssueDetailModalProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-primary/80 backdrop-blur-sm animate-appear">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col relative animate-zoom-in">
        ...
        <div className="h-64 bg-gray-200 relative shrink-0 group">
          ...
          {isAdmin && !validationResult && !isValidating && (
             <button 
                ...
                className="absolute top-4 left-4 bg-white/90 backdrop-blur-md hover:bg-white text-primary px-3 py-1.5 rounded-xl text-xs font-bold shadow-lg flex items-center gap-2 transition transform hover:scale-105"
             >
                <ScanEye size={16} /> Validar Imagen con IA
             </button>
          )}
        ...
        <div className="overflow-y-auto p-6 space-y-6 custom-scrollbar animate-fade-in">
          ...
          <div className="flex flex-wrap gap-4 text-sm text-gray-500 border-b border-gray-100 pb-6">
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border font-bold uppercase text-xs ${getStatusColor(issue.status)}`}>
               {getIcon(issue.status)} {issue.status}
            </div>
            ...
            <button 
              ...
              className="flex items-center gap-2 text-primary hover:text-blue-700 hover:underline ml-auto font-bold transition"
            >
          ...
          {/* Description */}
          <div>
            <h3 className="font-bold text-gray-800 text-lg mb-2">Descripción</h3>
          ...
          {isAdmin ? (
            <div className="mt-8 rounded-2xl border-2 border-primary bg-white overflow-hidden shadow-lg animate-fade-in ring-4 ring-primary/5">
                {/* Admin Header */}
                ...
                <div className="p-6 bg-slate-50/50 space-y-5">
                    <div>
                        ...
                        <textarea 
                           ...
                           className="w-full p-4 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary outline-none bg-white min-h-[120px] text-gray-700 shadow-sm transition"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end pt-2">
                        <div>
                           <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">Actualizar Estado</label>
                           <div className="relative">
                              <select 
                                 value={status}
                                 onChange={(e) => setStatus(e.target.value as IssueStatus)}
                                 className="w-full p-3 pl-4 border border-gray-300 rounded-xl text-sm bg-white focus:ring-2 focus:ring-primary outline-none font-bold text-gray-700 appearance-none shadow-sm cursor-pointer"
                              >
            ...
            <div className={`rounded-2xl border-2 overflow-hidden transition-colors ${issue.adminResponse ? 'border-blue-100 bg-blue-50' : 'border-gray-100 bg-gray-50'}`}>
                <div className="px-6 py-4 border-b border-black/5 flex items-center gap-3">
                    <div className="p-2 rounded-lg bg-blue-600 text-white">
                       <Shield size={20} />
                    </div>
                    <div>
                       <h3 className="font-bold text-gray-800">Respuesta Oficial</h3>
                       <p className="text-xs text-gray-500">Ayuntamiento de Sevilla</p>
                    </div>
                </div>
                ...
                <div className="p-6">
                     {issue.adminResponse ? (
                        <div className="relative">
                           <MessageSquare className="absolute -top-2 -left-2 text-primary/10 w-12 h-12 rotate-12" />
                           <p className="text-gray-700 relative z-10 italic">"{issue.adminResponse}"</p>
