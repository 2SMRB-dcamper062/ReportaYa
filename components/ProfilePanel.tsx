...
const ProfilePanel: React.FC<ProfilePanelProps> = ({ user, issues, onUpdateUser }) => {
    ...
    const initialSelectedTagId =
      user.profileTag && (ALL_SHOP_ITEMS.find(i => i.id === user.profileTag) || EXCLUSIVE_BADGES.find(i => i.id === user.profileTag))
        ? (user.profileTag || '')
        : '';
    const [selectedTagId, setSelectedTagId] = useState(initialSelectedTagId);
    const fileInputRef = useRef(null as HTMLInputElement | null);
    const [toastMsg, setToastMsg] = useState(null as string | null);
  ...
  return (
    <div className="max-w-5xl mx-auto animate-fade-in space-y-8 pb-10">
      ...
      {/* Modern Header Card */}
      <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-xl overflow-hidden relative">
        {/* Dynamic Background */}
        ...
        <div className="px-8 pb-6 relative">
          <div className="flex flex-col md:flex-row items-start md:items-end justify-between -mt-16 mb-4">
            ...
            <div className="flex items-end gap-6">
                ...
                <div className="mb-2 hidden md:block">
                        <div>
                            <h1 className="text-3xl font-black text-gray-800 dark:text-slate-100 flex items-center gap-2">
          ...
          <div className="md:hidden mt-2 mb-4">
               <div>
                    <h1 className="text-2xl font-black text-gray-800 dark:text-slate-100 flex items-center gap-2">
                        <span>{user.name}</span>
                        {user.premium && (
                    ...
                    <p className="text-gray-500 dark:text-slate-300 text-sm">{profileTagItem?.name || user.profileTag || (user.role === UserRole.ADMIN ? 'Administrador Municipal' : 'Ciudadano activo')}</p>
               </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
                    <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800">
                        {/* Recompensas */}
                <h3 className="font-bold text-gray-400 dark:text-slate-400 text-xs uppercase tracking-wider mb-4">Información Personal</h3>
                <div className="space-y-4">
                    {isEditing ? (
                        <div className="space-y-4">
                            <div>
                                <label className="text-xs text-gray-400 dark:text-slate-400">Nombre</label>
                                <input value={name} onChange={(e) => setName(e.target.value)} className="w-full p-3 border rounded-xl bg-white dark:bg-slate-950/40 dark:border-slate-700 dark:text-slate-100" />
                            </div>
                            <div>
                                <label className="text-xs text-gray-400 dark:text-slate-400">Tag de perfil (adquiridos)</label>
                                <select value={selectedTagId} onChange={(e) => setSelectedTagId(e.target.value)} className="w-full p-3 border rounded-xl bg-white dark:bg-slate-950/40 dark:border-slate-700 dark:text-slate-100">
                                    ...
                                    {unlockedDynamicBadges.map(d => (
                                        <option key={d.id} value={d.id}>{d.name}{(user.inventory||[]).includes(d.id) ? ' (Adquirido)' : ''}</option>
                                                        ...
                                                        {/* Tema movido a Ajustes */}
                            <div>
                                <label className="text-xs text-gray-400 dark:text-slate-400">Email</label>
                                <div className="p-3 bg-gray-50 dark:bg-slate-800/50 rounded">{user.email || ''}</div>
                            </div>
                            <div>
                                <label className="text-xs text-gray-400 dark:text-slate-400">Rol</label>
                                <div className="p-3 bg-gray-50 dark:bg-slate-800/50 rounded">{user.role === UserRole.ADMIN ? 'Administrador' : 'Ciudadano'}</div>
                            </div>
                        </div>
                    ) : (
                        <>
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <p className="text-xs text-gray-400 dark:text-slate-400">Rol</p>
                                    <p className="text-sm font-semibold capitalize text-slate-800 dark:text-slate-100">{user.role === UserRole.ADMIN ? 'Administrador' : 'Ciudadano'}</p>
                                </div>
                            </div>
                        </>
                    )}
            ...
            {/* Owned Badges */}
            <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800">
                <h3 className="font-bold text-gray-400 dark:text-slate-400 text-xs uppercase tracking-wider mb-4">Insignias</h3>
                    ...
                    {ownedBadges.length === 0 && (
                        <div className="text-sm text-gray-400 dark:text-slate-400">No tienes insignias aún.</div>
                    )}
                </div>
            </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800">
                <h3 className="font-bold text-gray-400 dark:text-slate-400 text-xs uppercase tracking-wider mb-4">Estadísticas</h3>
                <div className="grid grid-cols-2 gap-4">
                     <div className="p-4 bg-gray-50 dark:bg-slate-800/50 rounded-xl text-center">
                         ...
                         <span className="text-xs text-gray-500 dark:text-slate-300 font-medium">Reportes</span>
                     </div>
                     <div className="p-4 bg-green-50 dark:bg-emerald-900/20 rounded-xl text-center">
                         ...
                         <span className="text-xs text-green-700 dark:text-emerald-300 font-medium">Resueltos</span>
                     </div>
                </div>
            </div>
        </div>
